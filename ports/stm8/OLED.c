...
const u8 F16x16_Idx[] = 
{
	//"经度纬测距离周长面积度高坡亩仪搜索卫星大小查记录删除系统值设单价公顷元询每电池压经纬°㎡型号版本剩余空间返回退出"
        "经度纬时间日期"
};
...
//xxf生成字模
        ...
        0x22,0x67,0x22,0x12,0x12,0x12,0x00,0x02,0x02,0x02,0xFF,0x02,0x12,0x22,0x1E,0x00,/*"纬",0*/
...
//清除某一区域  @add by @xxf
//(x1,y1),   (x2,y2)------y2  for  reserve
void LCD_CLS_Rect_area(u8 x1,u8 y1,u8 x2,u8 y2)
{
    u8 x;
    LCD_Set_Pos(x1,y1);
    for(x=x1;x<x2;x++)
...
//}
void LCD_DLY_ms(u16 ms){
atomTimerDelay((u32)ms);
}
void LCD_Init(void)        
{
  //DDRA=0XFF;
  //   oled  gpio  init
  GPIO_Init( GPIOB , GPIO_PIN_5 , GPIO_MODE_OUT_PP_HIGH_SLOW );//LCD_SCL
  GPIO_Init( GPIOB , GPIO_PIN_3 , GPIO_MODE_OUT_PP_HIGH_SLOW );//LCD_SDA
  GPIO_Init( GPIOB , GPIO_PIN_1 , GPIO_MODE_OUT_PP_HIGH_SLOW );//LCD_RST
  GPIO_Init( GPIOE , GPIO_PIN_5 , GPIO_MODE_OUT_PP_HIGH_SLOW );//LCD_DC
